...
class host_tracker (EventMixin):
  ...
  def _handle_openflow_ConnectionUp (self, event):
    ...
    m.actions.append(of.ofp_action_output(port=of.OFPP_CONTROLLER))
    event.connection.send(m)
    log.debug("Installing flow for ICMP ping responses")
    msgICMP = of.ofp_flow_mod()
    msgICMP.priority = 1 # Higher than normal
    msgICMP.actions.append(of.ofp_action_output(port=of.OFPP_CONTROLLER))
    event.connection.send(msgICMP)
